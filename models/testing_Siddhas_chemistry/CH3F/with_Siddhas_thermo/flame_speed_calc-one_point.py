...
directory = '/work/westgroup/nora/Code/projects/PFAS/models/testing_Siddhas_chemistry/CH3F/with_Siddhas_thermo/cantera/chem_annotated.yaml'
...
for i in range(len(mole_frac_list)): 
    try: 
        ...
        try:
            if i!=0:
                d = f'data/{mole_frac_list[i-1]}_one_point.csv'
                if os.path.exists(d):  
        ...
        pd = sltn.to_pandas()
        pd.to_csv(f'data/{x}_one_point', index=False)
...
with open('speeds_one_point.csv', 'w+') as g:
    g.write(directory)
