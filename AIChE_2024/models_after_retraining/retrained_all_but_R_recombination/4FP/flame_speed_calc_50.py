...
for i in  range(len(mole_fractions)):
    ...
    try: 
        ...
        mole_frac_dict = {'C3H2F4(1)': x, 'O2(3)':1, 'N2':1} # 1/(1+1) = .50
...
#         phi = (12.9)*(x/(x+1+3.76))
        ...
        try:
            if i!=0:
                d = f'data_50/{species_count}/{mole_fractions[i-1]}.csv'
                if os.path.exists(d):  
        ...
        df1 = sltn.to_pandas()
        df1.to_csv(f'data_50/{species_count}/{phi}.csv', index=False)            
...
with open(f'flamespeeds_{species_count}_species_50.csv', 'w+') as g:
