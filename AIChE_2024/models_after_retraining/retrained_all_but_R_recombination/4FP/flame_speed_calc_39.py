...
therefore phi = (F/M ac) / (F/M stoich) = (x/(x+1+3.76))/(0.4/(0.4+1+3.76)) = (12.9)*(x/(x+1+3.76)) #is this wrong?? why am I doing molar fraction? 
...
To = 298
Po = 1e5 # ct.one_atm
gas = ct.Solution('./chemkin/copies/copy_chem0171.cti')
species_count = 171
mole_fractions = list(np.linspace(0.2, 1.0, 25))   #mole fraction of tetra
...
for i in  range(len(mole_fractions)):
    ...
    try: 
        ...
        mole_frac_dict = {'C3H2F4(1)': x, 'O2(3)':1, 'N2':1.56} # 1/(1+1.56) = .39
...
#         phi = (12.9)*(x/(x+1+3.76))
        ...
        try:
            if i!=0:
                d = f'data_39/{species_count}/{mole_fractions[i-1]}.csv'
                if os.path.exists(d):  
        ...
        Su = flame.velocity[0]
        results[phi] = Su
        print(f'at {phi} phi, velocity is {Su}')
        sltn = flame.to_solution_array()
        df1 = sltn.to_pandas()
        df1.to_csv(f'data_39/{species_count}/{phi}.csv', index=False)            
...
with open(f'flamespeeds_{species_count}_species_39.csv', 'w+') as g:
