...
directory = f'/work/westgroup/nora/Code/projects/halogens/refrigerants/blends/Papas_Zhang_blends/difluoromethane/chemkin/copies/{folder}'

gas = ct.Solution(directory)


mole_fractions = [1.0]   #mole fraction of CH2F2
...
for i in  range(len(mole_fractions)):
    ...
    try: 
        ...
        try:
            if i!=0:
                d = f'data/{mole_fractions[i-1]}_one_point_{folder}.csv'
                if os.path.exists(d):  
        ...
        df1 = sltn.to_pandas()
        df1.to_csv(f'data/{x}_one_point_{folder}.csv', index=False)            
