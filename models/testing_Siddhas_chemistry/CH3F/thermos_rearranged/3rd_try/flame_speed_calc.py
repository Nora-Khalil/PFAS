...
directory = '/work/westgroup/nora/Code/projects/PFAS/models/testing_Siddhas_chemistry/CH3F/thermos_rearranged/3rd_try/cantera/chem_annotated.yaml'
...
for i in range(len(mole_frac_list)): 
    try: 
        ...
        try:
            if i!=0:
                d = f'data/{mole_frac_list[i-1]}_114.csv'
                if os.path.exists(d):  
        ...
        pd = sltn.to_pandas()
        pd.to_csv(f'data/{x}_114.csv', index=False)
...
with open('speeds_114.csv', 'w+') as g:
    g.write(directory)
