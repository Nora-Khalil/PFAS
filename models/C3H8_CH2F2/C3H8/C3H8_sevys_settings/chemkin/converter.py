...
fullpath = '/work/westgroup/nora/Code/projects/PFAS/models/C3H8_CH2F2/C3H8/C3H8_sevys_settings/chemkin/'
...
def convert(full_path_to_chemkin_folder):
    ...
    command = f'scp {file_name} copies/copy_98_{file_name}'
    os.system(command)
    ...
    #this is a string of the output when I try to convert this file to a cti file. Will probably produce an error
        output = getoutput( f'ck2cti --input=copy_98_{file_name} --transport=tran.dat') 
        ...
        if re.search('PASSED',output):
            ...
            if re.search('Encountered\sunmarked\sduplicate\sreaction',output):
                ...
                with open(f'./copy_98_{file_name}','r') as f:
                    data = f.readlines()
                ...
                with open(f'copy_98_{file_name}','w+') as f: 
                    for l in data: 
                        f.write(l)
                        x==0
